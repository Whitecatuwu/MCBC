...
def ignorepath(pathlist:dict,src,dst,purge=False):
    def _ignore(path_src,names):
        ...
        for fi in filter(lambda x:not path.isabs(x),pathlist["D"]):
            delete_list.extend(fnmatch.filter(names, fi))
        ...
        for fi in filter(lambda x:not path.isabs(x),pathlist["M"]):
            modify_list.extend(fnmatch.filter(names, fi))
        ...
        if purge:
            ...
            if path.exists(path_dst):
                for d in list(scandir(path_dst)):
                    if (d.name in delete_list) or ( (d.name not in names) and (path.join(path_dst,d.name) not in [dst+p for p in pathlist["M"]]) ):
                        try:
